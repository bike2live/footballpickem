...
//console.log(JSON.stringify(results));
            ...
            var utc = new Date();
            var today = new Date(utc.getTime() - (utc.getTimezoneOffset() * 60000));
            today = new Date();
            ...
            angular.forEach($scope.schedule.games,  function(game, key) {
                ...
                var gameDate = new Date(Date.parse(game.gameDate.replace(' ', 'T')));
                var closeDate = new Date(Date.parse(game.closeDate.replace(' ', 'T')));
                var showUntilDate = new Date(Date.parse(game.showUntilDate.replace(' ', 'T')));
...
$scope.today = today;
$scope.gameDate = $scope.currentGame.closeDate.replace(' ', 'T');
            //$scope.getUserScore();
    ...
    $scope.hasScore = function(userGuess) {
       // return "fa-check text-success";
        console.log(' show score? ', (userGuess.byuScore !== 0 && userGuess.oppScore !== 0));
