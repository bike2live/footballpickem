...
/*
    $scope.getUserScore = function() {
        return Data.get('userScore/' + $scope.currentGame.id)
            .then(function (results) {
//                Data.toast(results);
                var usrScore = results.userScore;
                if (usrScore) {
                    $scope.userScore = usrScore;
                    if (usrScore.byu != '') {
                        $scope.userScore.byu = parseInt(usrScore.byu);
                    }
                    if (usrScore.opponent != '') {
                        $scope.userScore.opponent = parseInt(usrScore.opponent);
                    }
                    $log.debug('userScore:' + JSON.stringify($scope.userScore));
                }
            }, function (reason) {
                Data.toast(reason);
            });
