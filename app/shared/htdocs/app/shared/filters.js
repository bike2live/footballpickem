app.filter('timeFilter', ['$filter', function ($filter) {
    return function (input) {
        ...
        var gDate = new Date(Date.parse(input.replace(' ', 'T')));
...
app.filter('dateFilter', ['$filter', function ($filter) {
    return function (input) {
        if (input == null) { return ""; }
        return $filter('date')(new Date(Date.parse(input.replace(' ', 'T'))), "mediumDate");
    }
}]);
...
app.filter('weekDayFilter', ['$filter', function ($filter) {
    return function (input) {
        if (input == null) { return ""; }
        return $filter('date')(new Date(Date.parse(input.replace(' ', 'T'))), "EEEE");
    }
}]);
