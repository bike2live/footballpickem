...
class DbHandler
{
    ...
    public function insertUserScore($userScore) {
        $sql = "INSERT INTO scores (uid, gameId, byuScore, oppScore, updated) values (:uid, :gameId, :byuScore, :oppScore, UTC_TIMESTAMP())";
        try {
            $pdo = $this->getPDOConnection();
            $stmt = $pdo->prepare($sql);
    ...
    public function updateUserScore($userScore) {
        $sql = "UPDATE scores set byuScore=:byuScore, oppScore=:oppScore, updated=UTC_TIMESTAMP() WHERE id=:id";
        try {
            $pdo = $this->getPDOConnection();
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam("byuScore", $userScore->byuScore);
